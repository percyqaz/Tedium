...
module Commands =
    ...
    let dispatch_internal_command (state: State, command: string) : unit =
        ...
        | "edit" -> match state.Selected with Some item -> Operations.edit(state.List, item) | None -> Operations.edit_fm(state.List)
        | "delete" -> delete(state)
        | "desc" -> match state.Selected with Some item -> Operations.edit_contents(item) | None -> Operations.edit_fm(state.List)
        | "rename" -> match state.Selected with Some item -> Operations.edit_name(state.List, item) | None -> ()
        | _ -> ()
    ...
    let send_text (state: State, text: string) : unit =
        ...
        if text <> "" then
            ...
            let index =
                match state.Selected with Some item -> state.List.Items.IndexOf(item) | None -> -1
